...
bool cmc_config_manager_init(void) {
    ...
    if (cmc_config.signature == CMC_CONFIG_SIGNATURE) {
        is_valid = cmc_config_manager_validate(&cmc_config);
    }
    
    // 2. Make the decision
    if (is_valid) {
        // Safe! Copy from Flash to active RAM
        memcpy(&cmc_config, flash_config, sizeof(cmc_config_t));
        return true;
    } else {
        // DANGER! Memory is corrupt or brand new.
        // The Manager tells the Defaults file to do its job:
        cmc_config_defaults_load(); 

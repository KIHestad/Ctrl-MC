...
static cmc_app_state_status_t check_flash_config(void) {
    if (flash_config->signature != CMC_CONFIG_SIGNATURE) {
        return CMC_APP_STATE_STATUS_INVALID_FLASH_SIGNATURE;
    }
    if (CMC_UTIL_CRC_CALCULATE_PAYLOAD(flash_config) != flash_config->crc) {
        return CMC_APP_STATE_STATUS_INVALID_FLASH_CRC;
    }
...
cmc_app_state_status_t cmc_config_manager_save_to_flash(const cmc_config_t* new_config) {
    if (new_config == NULL) {
        return CMC_APP_STATE_STATUS_ERROR_SAVE_TO_FLASH;
    }
...
cmc_app_state_status_t cmc_config_manager_load_from_flash(cmc_config_t* target) {
    if (target == NULL) {
        return CMC_APP_STATE_STATUS_ERROR_LOAD_FROM_FLASH;
    }
    ...
    if (target->signature != CMC_CONFIG_SIGNATURE) {
        return CMC_APP_STATE_STATUS_INVALID_RAM_SIGNATURE;
    }

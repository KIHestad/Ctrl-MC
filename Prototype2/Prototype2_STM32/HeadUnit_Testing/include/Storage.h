    ...
    public:
        ...
        class Data {
            public:
                ...
                Prop fuelLevelWhenRefueled; // stores fuel level in litres, only when refueled
        };
        ...
        void init() {
            ...
            data.fuelLevelWhenRefueled.address = 12; // occupies bytes 12,13,14,15
            // Note: STM32 EEPROM doesn't need begin() call
            // Read stored values
            ...
            EEPROM.get(data.fuelLevelWhenRefueled.address, data.fuelLevelWhenRefueled.value);
            // Validate values
            ...
            if (std::isnan(data.fuelLevelWhenRefueled.value)) data.fuelLevelWhenRefueled.value = 0.0;
        ...
        void resetFuelLevel(float litres) {
            data.fuelLevelWhenRefueled.value = litres;
            data.fuelTripCounter.value = 0.0;
            EEPROM.put(data.fuelLevelWhenRefueled.address, data.fuelLevelWhenRefueled.value);
            EEPROM.put(data.fuelTripCounter.address, data.fuelTripCounter.value);
        };

        // Save current values to EEPROM
        void saveAllTripCounters() {

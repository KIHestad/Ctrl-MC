/***  Ctrl-MC // An open source Motorcycle Controller Arduino project by KI Hestad: https://github.com/KIHestad/Ctrl-MC  ***/

class Image {

    private: 

        void displayImage(const unsigned char img[], ImagePosition imgPos, uint8_t width, uint8_t height) {
            if (imgPos == imgPosMenuCenter)
                display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2), 3+25-(height/2), img, width, height, SSD1306_WHITE);
        }

    public:
        void kiHestadLogo() {
            static const unsigned char PROGMEM img[] = {
                
            };
            display.clearDisplay();
            display.drawBitmap(0,0, img, DISPLAY_SCREEN_WIDTH, DISPLAY_SCREEN_HEIGHT, SSD1306_WHITE);
            display.display();
        }

        void ignOff() {
            static const unsigned char PROGMEM img[] = {
                // 'IGN_OFF', 46x50px
                
            };
            uint8_t width = 46;
            uint8_t height = 50;
            display.clearDisplay();
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),3, img, width, height, SSD1306_WHITE);
            display.display();
        }

        void ignOffToOn1() {
            static const unsigned char PROGMEM img[] = {
                // 'IGN_OFF_TO_ON1', 46x50px
                
            };
            uint8_t width = 46;
            uint8_t height = 50;
            display.clearDisplay();
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),3, img, width, height, SSD1306_WHITE);
            display.display();
        }
            
        void ignOffToOn2() {
            static const unsigned char PROGMEM img[] = {
                // 'IGN_OFF_TO_ON2', 46x50px
                
            };
            uint8_t width = 46;
            uint8_t height = 50;
            display.clearDisplay();
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),3, img, width, height, SSD1306_WHITE);
            display.display();
        }

        void ignOn() {
            static const unsigned char PROGMEM img[] = {
                // 'IGN_ON', 46x50px
                
            };
            uint8_t width = 46;
            uint8_t height = 50;
            display.clearDisplay();
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),3, img, width, height, SSD1306_WHITE);
            display.display();
        }

        void warning() {
            static const unsigned char PROGMEM img[] = {
                // 'Warning', 48x48px
                
            };
            uint8_t width = 48;
            uint8_t height = 48;
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),0, img, width, height, SSD1306_WHITE);
            display.display();
        }

        void retry() {
            static const unsigned char PROGMEM img[] = {
                // 'retry', 50x42px
                
            };
            uint8_t width = 50;
            uint8_t height = 42;
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),6, img, width, height, SSD1306_WHITE);
            display.display();
        }

        void stopwatch() {
            static const unsigned char PROGMEM img[] = {
                // 'stopwatch_v2', 26x31px
                0x00, 0x1e, 0x00, 0x00, 0x00, 0x3e, 0x00, 0x00, 0x00, 0x3e, 0x00, 0x00, 0x00, 0x1c, 0x00, 0x00, 
                0x00, 0x1c, 0x00, 0x00, 0x01, 0xff, 0xc3, 0x00, 0x07, 0xff, 0xf3, 0x80, 0x0f, 0x00, 0x7f, 0x80, 
                0x1c, 0x0c, 0x1e, 0x00, 0x38, 0x00, 0x0e, 0x00, 0x30, 0x00, 0x07, 0x00, 0x70, 0x00, 0x03, 0x00, 
                0x60, 0x00, 0x63, 0x80, 0xe0, 0x00, 0xc1, 0x80, 0xc0, 0x01, 0x81, 0x80, 0xc0, 0x03, 0x01, 0x80, 
                0xc0, 0x02, 0x01, 0x80, 0xf0, 0x0c, 0x05, 0x80, 0xe0, 0x00, 0x01, 0x80, 0xc0, 0x00, 0x01, 0x80, 
                0xc0, 0x00, 0x01, 0x80, 0xc0, 0x00, 0x01, 0x80, 0x60, 0x00, 0x03, 0x80, 0x60, 0x00, 0x03, 0x00, 
                0x30, 0x00, 0x07, 0x00, 0x38, 0x00, 0x0e, 0x00, 0x1c, 0x0c, 0x1c, 0x00, 0x0f, 0x00, 0x78, 0x00, 
                0x07, 0xff, 0xf0, 0x00, 0x01, 0xff, 0xc0, 0x00, 0x00, 0x7f, 0x00, 0x00
            };
            uint8_t width = 26;
            uint8_t height = 31;
            display.drawBitmap(0,((DISPLAY_SCREEN_HEIGHT/2)-(height/2))-DISPLAY_TEXT_CHAR_HEIGHT, img, width, height, SSD1306_WHITE);
            display.display();
        }

        void temperature() {
            static const unsigned char PROGMEM img[] = {
                // 'temperature', 24x48px
                
            };
            uint8_t width = 24;
            uint8_t height = 48;
            display.drawBitmap(0,((DISPLAY_SCREEN_HEIGHT/2)-(height/2))-DISPLAY_TEXT_CHAR_HEIGHT, img, width, height, SSD1306_WHITE);
            display.display();
        }

        void lightsOffBig(ImagePosition imgPos) {
            static const unsigned char PROGMEM img[] = {
                // 'L_OFF_BIG', 34x36px
                
            };
            displayImage(img, imgPos, 34, 36);
        }

        void lightsParkBig(ImagePosition imgPos) {
            static const unsigned char PROGMEM img[] = {
                // 'L_PARK_BIG', 60x36px
                
            };
            displayImage(img, imgPos, 60, 36);
        }

        void lightsLowBig(ImagePosition imgPos) {
            static const unsigned char PROGMEM img[] = {
                // 'L_LOWBEAM_BIG', 60x50px
                
            };
            displayImage(img, imgPos, 60, 50);
        }

        void lightsHighBig(ImagePosition imgPos) {
            static const unsigned char PROGMEM img[] = {
                // 'L_HIGHBEAM_BIG', 60x50px
                
            };
            displayImage(img, imgPos, 60, 50);
        }

        void menuSelectFrame(bool left, bool right) {
            static const unsigned char PROGMEM img[] = {
                // 'MenuCheckUnselected', 50x50px
                
            };
            uint8_t width = 50;
            uint8_t height = 50;
            if (left) {
                uint8_t leftPosition = (DISPLAY_SCREEN_WIDTH/2)-6-width;
                display.drawBitmap(leftPosition, 3, img, width, height, SSD1306_WHITE);
            } 
            if (right) {
                uint8_t leftPosition = (DISPLAY_SCREEN_WIDTH/2)+6;
                display.drawBitmap(leftPosition, 3, img, width, height, SSD1306_WHITE);
            } 

        }
        
        void menuSelectFrameContentInverse(bool left, bool right) {
            static const unsigned char PROGMEM img[] = {
                // 'MenuCheckSelected', 50x50px
                
            };
            uint8_t width = 50;
            uint8_t height = 50;
            if (left) {
                uint8_t leftPosition = (DISPLAY_SCREEN_WIDTH/2)-6-width;
                display.drawBitmap(leftPosition, 3, img, width, height, SSD1306_INVERSE);
            } 
            if (right) {
                uint8_t leftPosition = (DISPLAY_SCREEN_WIDTH/2)+6;
                display.drawBitmap(leftPosition, 3, img, width, height, SSD1306_INVERSE);
            } 

        }
        
        void menuSelectIconConfirm(bool left, bool right) {
            static const unsigned char PROGMEM img[] = {
                // 'MenuCheckIconX', 19x19px
                0x20, 0x00, 0x80, 0x70, 0x01, 0xc0, 0xf8, 0x03, 0xe0, 0x7c, 0x07, 0xc0, 0x3e, 0x0f, 0x80, 0x1f, 
                0x1f, 0x00, 0x0f, 0xbe, 0x00, 0x07, 0xfc, 0x00, 0x03, 0xf8, 0x00, 0x01, 0xf0, 0x00, 0x03, 0xf8, 
                0x00, 0x07, 0xfc, 0x00, 0x0f, 0xbe, 0x00, 0x1f, 0x1f, 0x00, 0x3e, 0x0f, 0x80, 0x7c, 0x07, 0xc0, 
                0xf8, 0x03, 0xe0, 0x70, 0x01, 0xc0, 0x20, 0x00, 0x80
            };
            uint8_t width = 19;
            uint8_t height = 19;
            uint8_t y = 3 + 11;
            if (left) {
                uint8_t x = ((DISPLAY_SCREEN_WIDTH/2)-6-25)-(width/2);
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            }
            if (right) {
                uint8_t x = ((DISPLAY_SCREEN_WIDTH/2)+6+25)-(width/2);
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            }

        }

        void menuSelectIconReject(bool left, bool right) {
            static const unsigned char PROGMEM img[] = {
                // 'MenuCheckIconCheck', 19x18px
                0x00, 0x00, 0x80, 0x00, 0x01, 0xc0, 0x00, 0x03, 0xe0, 0x00, 0x03, 0xe0, 0x00, 0x07, 0xc0, 0x00, 
                0x0f, 0x80, 0x00, 0x1f, 0x00, 0x00, 0x1f, 0x00, 0x00, 0x3e, 0x00, 0x20, 0x7c, 0x00, 0x70, 0xf8, 
                0x00, 0xf8, 0xf8, 0x00, 0x7d, 0xf0, 0x00, 0x3f, 0xe0, 0x00, 0x1f, 0xc0, 0x00, 0x0f, 0xc0, 0x00, 
                0x07, 0x80, 0x00, 0x03, 0x00, 0x00
            };
            uint8_t width = 19;
            uint8_t height = 18;
            uint8_t y = 3 + 11;
            if (left) {
                uint8_t x = ((DISPLAY_SCREEN_WIDTH/2)-6-25)-(width/2);
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            }
            if (right) {
                uint8_t x = ((DISPLAY_SCREEN_WIDTH/2)+6+25)-(width/2);
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            }

        }

        void menuSelectIconOn(bool left, bool right) {
            static const unsigned char PROGMEM img[] = {
                // 'MenuCheckOn', 30x32px
                
            };
            uint8_t width = 30;
            uint8_t height = 32;
            uint8_t y = 3 + (50/2) - (height/2); // (50/2) = Middle of Checkbox frama size
            if (left) {
                uint8_t x = (DISPLAY_SCREEN_WIDTH/2)-6-(50/2)-(width/2); // (50/2) = Middle of Checkbox frama size
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            } 
            if (right) {
                uint8_t x = (DISPLAY_SCREEN_WIDTH/2)+6+(50/2)-(width/2); // (50/2) = Middle of Checkbox frama size 
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            } 

        }

        void menuSelectIconOff(bool left, bool right) {
            static const unsigned char PROGMEM img[] = {
                // 'MenuCheckOff', 30x32px
                
            };
            uint8_t width = 30;
            uint8_t height = 32;
            uint8_t y = 3 + (50/2) - (height/2); // (50/2) = Middle of Checkbox frama size
            if (left) {
                uint8_t x = (DISPLAY_SCREEN_WIDTH/2)-6-(50/2)-(width/2); // (50/2) = Middle of Checkbox frama size
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            } 
            if (right) {
                uint8_t x = (DISPLAY_SCREEN_WIDTH/2)+6+(50/2)-(width/2); // (50/2) = Middle of Checkbox frama size 
                display.drawBitmap(x, y, img, width, height, SSD1306_WHITE);
            } 

        }

        void indicatorRight() {
            static const unsigned char PROGMEM img[] = {
                // 'indicator_right', 81x49px
                
            };
            uint8_t width = 81;
            uint8_t height = 49;
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),((DISPLAY_SCREEN_HEIGHT/2)-(height/2)), img, width, height, SSD1306_WHITE);
            display.display();
        }

        void indicatorLeft() {
            static const unsigned char PROGMEM img[] = {
                // 'indicator_right', 81x49px
                
            };
            uint8_t width = 81;
            uint8_t height = 49;
            display.drawBitmap((DISPLAY_SCREEN_WIDTH/2)-(width/2),((DISPLAY_SCREEN_HEIGHT/2)-(height/2)), img, width, height, SSD1306_WHITE);
            display.display();
        }

};

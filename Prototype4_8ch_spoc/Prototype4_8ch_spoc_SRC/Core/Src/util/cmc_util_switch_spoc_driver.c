...
// byte). Datasheet Fig. 37/38 confirm the byte-order rule; this flag is the only place the
// PHYSICAL chip identity needs to match the schematic — verify with a logic analyzer at bring-up.
...
// Approximate microsecond busy-wait for current-sense mux settling (tsIS(CC), max 60us per datasheet);
// TODO: replace with a calibrated hardware timer if tighter accuracy is needed after bring-up
...
void cmc_util_switch_spoc_init(void) {
    ...
    spoc_xfer(build_out_write(0U), build_out_write(0U), &discard0, &discard1);
    for (uint8_t ch = 0U; ch < CMC_CONFIG_HW_OUT_COUNT; ch++) { s_is_raw[ch] = 0U; }
}
